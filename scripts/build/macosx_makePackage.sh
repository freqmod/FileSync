...
if [[ $BUILDTYPE = "Core" ]] || [[ $BUILDTYPE = "Client" ]]; then
    QUASSEL_DMG="FileSync${BUILDTYPE}_MacOSX-universal_${QUASSEL_VERSION}.dmg"
    mkdir $PACKAGETMPDIR
    if [[ $BUILDTYPE = "Client" ]]; then
	cp -r ${WORKINGDIR}FileSync\ Client.app ${PACKAGETMPDIR}/
	${SCRIPTDIR}/macosx_DeployApp.py "${PACKAGETMPDIR}/FileSync Client.app"
    else
	cp ${WORKINGDIR}filesync ${PACKAGETMPDIR}/
	${SCRIPTDIR}/macosx_DeployApp.py --nobundle ${PACKAGETMPDIR}
    fi
    hdiutil create -srcfolder ${PACKAGETMPDIR} -format UDBZ -volname "FileSync ${BUILDTYPE} - ${QUASSEL_VERSION}" "${WORKINGDIR}FileSync${BUILDTYPE}_MacOSX-universal_${QUASSEL_VERSION}.dmg" >/dev/null
    rm -rf ${PACKAGETMPDIR}
else
